...
public class ChessBoard : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            if (blackPlayer is LukasEngine engine)
            {
                userInterfaceComponent.UpdateUIEval(true,new Dictionary<Move, float>(engine.moveEvaluation), engine.currentDepth);
            } 
            else if (whitePlayer is LukasEngine engine2)
            {
                userInterfaceComponent.UpdateUIEval(false, new Dictionary<Move, float>(engine2.moveEvaluation), engine2.currentDepth);
            } 
        }
